...
PROGMEM uchar measuring_message[]	= "Starting generation... (press " STR(PASS_LENGTH) "x caps lock)";
PROGMEM uchar finish_message[]		= "\nNew password saved: ";
...
void caps_toggle() {
    if(capsState == CAPS_COUNTING) {    
        ...
        if(capsCount >= PASS_LENGTH) { // enough characters generated
			...
			for(i=0; i<PASS_LENGTH; i++)	messageBuffer[i + sizeof(finish_message) - 1] = messageBuffer[i];	// move password after finish_message
			messageBuffer[i + sizeof(finish_message) - 1] = 0;						// terminate the string
			
			// Type a message to the PC what the new password is
			memcpy_P(messageBuffer, finish_message, sizeof(finish_message)-1);
            messagePtr = 0;
            messageState = STATE_SEND;
