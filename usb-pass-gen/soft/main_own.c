...
#define PASS_LENGTH 4	// password length for generated password
...
PROGMEM uchar intro_message[]		= "Chose option (toggle once):\n\n";
PROGMEM uchar genpass_message[]		= "CAPSLOCK: Generate new password and save it to EEPROM.";
PROGMEM uchar genchng_message[]		= "NUMLOCK:  CAPSLOCK and change windows password.";
PROGMEM uchar confirm_message[]		= "Sure? (CAPSLOCK=Yes, NUMLOCK=NO)";
PROGMEM uchar measuring_message[]	= "Press " STR(PASS_LENGTH) "x CAPSLOCK...\n";
PROGMEM uchar newpass_message[]		= "New password: ";
PROGMEM uchar done_message[]		= "Done!";

// The buffer needs to accommodate the messages above and the password
#define MSG_BUFFER_SIZE 55
...
PROGMEM char usbHidDescriptor[USB_CFG_HID_REPORT_DESCRIPTOR_LENGTH] = {
...
uchar buildReport() {
    ...
    if(messageCharNext) { // send a keypress
        ...
        if(ch >= '0' && ch <= '9') {
            ...
            case '(':
				keyboard_report.modifier = (LED_state & 2) ? 0 : MOD_SHIFT_LEFT;
				keyboard_report.keycode[0] = 37;  			// 8 + shift = (
                break;
            case ')':
				keyboard_report.modifier = (LED_state & 2) ? 0 : MOD_SHIFT_LEFT;
				keyboard_report.keycode[0] = 38;			// 9 + shift = )
                break;
            case ':':
				keyboard_report.modifier = (LED_state & 2) ? 0 : MOD_SHIFT_LEFT;
				keyboard_report.keycode[0] = 0x37;			// . + shift = :
                break;
            }
        }
...
void caps_toggle() {
    if(capsState == CAPS_COUNTING) {    
        ...
        if(capsCount >= PASS_LENGTH) { // enough characters generated
			...
			for(i=0; i<PASS_LENGTH; i++)	messageBuffer[i + sizeof(newpass_message) - 1] = messageBuffer[i]; // move password after newpass_message
			memcpy_P(messageBuffer, newpass_message, sizeof(newpass_message) - 1);
			messageBuffer[sizeof(newpass_message) + PASS_LENGTH - 1]	= 0;
