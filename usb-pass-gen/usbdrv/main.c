...
PROGMEM uchar finish_message[] = "Following password was saved: ";
...
void caps_toggle() {
	...
	if(capsState == CAPS_COUNTING) {    
        ...
        if(capsCount >= PASS_LENGTH) { // enough characters generated
			...
			for(i=0; i<PASS_LENGTH; i++) // move password after finish_message
			messageBuffer[i + sizeof(finish_message)-1] = messageBuffer[i];
			
			messageBuffer[i] = 0; // terminate the string
